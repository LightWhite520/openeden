...
object Sha256 {
    ...
    private fun digest(input: ByteArray): ByteArray {
        ...
        for (index in 0 until 8) message[paddedLength - 1 - index] = (bitLength ushr (index * 8)).toByte()
        ...
        for (offset in message.indices step 64) {
            for (index in 0 until 16) {
                ...
                schedule[index] = ((message[base].toInt() and 0xff) shl 24) or ((message[base + 1].toInt() and 0xff) shl 16) or ((message[base + 2].toInt() and 0xff) shl 8) or (message[base + 3].toInt() and 0xff)
            }
            ...
            for (index in 0 until 64) {
                val temporary1 = h + (e.rotateRight(6) xor e.rotateRight(11) xor e.rotateRight(25)) + ((e and f) xor (e.inv() and g)) + ROUND_CONSTANTS[index] + schedule[index]
                val temporary2 = (a.rotateRight(2) xor a.rotateRight(13) xor a.rotateRight(22)) + ((a and b) xor (a and c) xor (b and c))
                h = g; g = f; f = e; e = d + temporary1; d = c; c = b; b = a; a = temporary1 + temporary2
            }
            state[0] += a; state[1] += b; state[2] += c; state[3] += d; state[4] += e; state[5] += f; state[6] += g; state[7] += h
        }
        return ByteArray(state.size * 4).also { result ->
            state.forEachIndexed { index, value ->
                val offset = index * 4
                result[offset] = (value ushr 24).toByte(); result[offset + 1] = (value ushr 16).toByte(); result[offset + 2] = (value ushr 8).toByte(); result[offset + 3] = value.toByte()
            }
        }
    }
